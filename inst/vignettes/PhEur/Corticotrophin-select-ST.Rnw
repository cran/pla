...
plaModel <- plaCRD(Data, dilutionRatio = 4, factor = factor, echoData = FALSE)
Fits <- pla.fit(plaModel@data, design = "crd", sampleLabels = c("S", "T"),
                dr = 4, factor = factor,
...
plots <- pla.plots(plaModel@data, design = "crd", sampleLabels = c("S", "T"))
@ 
