...
@ 
<<eval=TRUE,echo=FALSE>>=
Data    <- readAssayTable("data/Agar-missing.txt",
                          rows = "Measurements", 
...
PLOTS <- c("restricted", "unrestricted", "twoway",
           "stepadjusted", "qplot", "residualsBoxSample")
## "residualsSample": Fejl i data.frame(..., check.names = FALSE) : 
##                    arguments imply differing number of rows: 32, 36
## plots <- plot(plaModel, mfrow = c(2, 3), plots = PLOTS)
plots <- plot(plaModel)
@ 
