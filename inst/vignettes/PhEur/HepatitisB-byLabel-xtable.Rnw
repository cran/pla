...
library(xtable)
source("R/xtables.R")
Data <- readAssayTable("data/HepatitisB_byLabel.txt", fun = log, echoTitle = FALSE)
@ 
<<eval=TRUE,echo=FALSE>>=
factor <- c(20, 100, NA)
names(factor) <- c("Potency", "Rel.\ to Ass.", "Rel.\ to Est.")
plaModel <- pla(Data, factor = factor, dilutionRatio = 2,
