...
library(xtable)
source("R/xtables.R")
Data <- readAssayTable("data/Vancomycin_frame_NA.txt", echoTitle = FALSE)
Data@design <- "rbd"
@
<<eval=TRUE,echo=FALSE>>=
plaModel <- pla(Data, dilutionRatio = 2.25, imputeMissing = TRUE,
                version = 1, trace = FALSE, echoData = FALSE, design = "RBD")
Fits <- fit(plaModel)
@
<<echo=False, results=tex>>=
xtablesAssay(data = Data, model = plaModel, fits = Fits, imputeMissing = TRUE)
@
