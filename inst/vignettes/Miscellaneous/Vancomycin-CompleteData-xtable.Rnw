...
@
<<eval=TRUE,echo=FALSE>>=
plaModel <- plaRBD(Data, dilutionRatio = 2.25, echoData = FALSE)
Fits <- fit(plaModel)
@
...
plots <- plot(plaModel, plots = "twoway", 
              pch = 32 + 16 + as.numeric(unlist(c(plaModel@data["Replicate"]))),
              mfrow = c(1, 1))
@
...
\begin{figure}[h]
  \centering
<<echo=FALSE, eval=TRUE, fig=TRUE, width=6, height=6>>=
plots <- plot(plaModel, plots = "stepadjusted", 
              pch = 32 + 16 + as.numeric(unlist(c(plaModel@data["Replicate"]))),
              mfrow = c(1, 1))
@
\caption{\textit{Step Adjusted Two way.}}
\end{figure}
@
\setkeys{Gin}{width=1.00\textwidth}
\begin{figure}[h]
  \centering
<<echo=FALSE, eval=TRUE, fig=TRUE, width=6, height=6>>=
plots <- plot(plaModel, plots = "residualsPlate", 
              pch = 32 + 16 + as.numeric(unlist(c(plaModel@data["Replicate"]))),
              mfrow = c(1, 1))
@
